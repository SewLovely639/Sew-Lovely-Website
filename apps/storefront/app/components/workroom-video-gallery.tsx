...
export function WorkroomVideoGallery({ videos: managedVideos = [], products = [] }: { videos?: SiteContent["workroomVideos"]; products?: CmsProduct[] }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => { if (entry.isIntersecting) { setNearViewport(true); observer.disconnect(); } }, { rootMargin: "240px 0px" });
  ...
  return <section ref={galleryRef} className="mx-auto max-w-[1440px] px-0 py-10 sm:px-8 lg:py-14" aria-label="Sew Lovely videos"><h2 className="sr-only">Sew Lovely workroom videos</h2><div className="flex gap-1 overflow-x-auto pb-3 sm:gap-4">{videos.map((video, index) => { const taggedProducts = video.productIds.map((id) => { const product = products.find((candidate) => candidate.id === id); return { id, name: product?.name ?? id.replace(/[-_]+/g, " ") }; }); return <div key={video.id} className="sew-editorial-tile group relative min-w-[calc(50%-0.125rem)] shrink-0 overflow-hidden bg-[#21161e] sm:min-w-[43%] md:min-w-[calc(25%-0.75rem)] lg:w-[calc(25%-0.75rem)] lg:flex-none"><button type="button" onClick={() => { setSoundOn(false); setActive(index); }} className="relative size-full text-left" aria-label={`Open ${video.label} video`}><video autoPlay muted loop playsInline preload={nearViewport ? "metadata" : "none"} onLoadedMetadata={(event) => cueClip(event.currentTarget, video.startAt)} className="size-full object-cover transition duration-500 group-hover:scale-105">{nearViewport && <source src={video.src} type="video/mp4" />}</video><span className="absolute inset-0 bg-gradient-to-t from-[#21161e]/75 via-transparent to-transparent transition duration-300 group-hover:from-[#21161e]/85" /><span className="absolute bottom-3 left-3 grid size-8 place-items-center rounded-full border border-white/70 bg-black/15 text-white opacity-0 transition duration-300 group-hover:opacity-100 sm:size-9"><Play className="ml-0.5 size-3 fill-current" /></span></button>{taggedProducts.length > 0 && <div className="absolute bottom-3 right-3 z-10"><Link href={`/products/${taggedProducts[0].id}`} onClick={() => trackShopClick(video.id, taggedProducts[0].id)} style={{ backgroundColor: video.buttonColor }} className="inline-flex border border-white/75 px-3 py-2 text-[10px] font-bold uppercase tracking-[0.14em] text-white shadow-[0_5px_18px_rgba(0,0,0,.18)] transition hover:brightness-110 focus:outline-none focus:ring-2 focus:ring-white">Shop now</Link></div>}</div>; })}</div>{typeof document !== "undefined" && viewer ? createPortal(viewer, document.body) : null}</section>;
}
