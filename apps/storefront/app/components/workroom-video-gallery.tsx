...
export function WorkroomVideoGallery({ videos: managedVideos = [], products = [] }: { videos?: SiteContent["workroomVideos"]; products?: CmsProduct[] }) {
  ...
  return <section className="mx-auto max-w-[1440px] px-5 py-10 sm:px-8 lg:py-14" aria-label="Sew Lovely videos"><h2 className="sr-only">Sew Lovely workroom videos</h2><div className="flex gap-3 overflow-x-auto pb-3 sm:gap-4">{videos.map((video, index) => { const taggedProducts = video.productIds.map((id) => { const product = products.find((candidate) => candidate.id === id); return { id, name: product?.name ?? id.replace(/[-_]+/g, " ") }; }); return <div key={video.id} className="group relative min-w-[58vw] shrink-0 aspect-[.78] overflow-hidden bg-[#21161e] sm:min-w-[31vw] lg:min-w-0 lg:w-[23%] lg:flex-none"><button type="button" onClick={() => { setSoundOn(false); setActive(index); }} className="relative size-full text-left" aria-label={`Open ${video.label} video`}><video autoPlay muted loop playsInline preload="metadata" onLoadedMetadata={(event) => cueClip(event.currentTarget, video.startAt)} className="size-full object-cover transition duration-500 group-hover:scale-105"><source src={video.src} type="video/mp4" /></video><span className="absolute inset-0 bg-gradient-to-t from-[#21161e]/75 via-transparent to-transparent transition duration-300 group-hover:from-[#21161e]/85" /><span className="absolute bottom-3 left-3 grid size-8 place-items-center rounded-full border border-white/70 bg-black/15 text-white opacity-0 transition duration-300 group-hover:opacity-100 sm:size-9"><Play className="ml-0.5 size-3 fill-current" /></span></button>{taggedProducts.length > 0 && <div className="absolute bottom-3 right-3 z-10"><button type="button" onClick={() => setShopOpen(shopOpen === index ? null : index)} style={{ backgroundColor: video.buttonColor }} className="border border-white/75 px-3 py-2 text-[10px] font-bold uppercase tracking-[0.14em] text-white shadow-[0_5px_18px_rgba(0,0,0,.18)] transition hover:brightness-110 focus:outline-none focus:ring-2 focus:ring-white" aria-expanded={shopOpen === index}>{video.buttonLabel}</button>{shopOpen === index && <div className="absolute bottom-[calc(100%+8px)] right-0 grid w-[min(240px,65vw)] gap-1 border border-[#21161e]/10 bg-[#fffafb] p-2 shadow-[0_16px_40px_rgba(30,16,26,.28)]"><p className="px-2 pb-1 pt-0.5 text-[9px] font-bold uppercase tracking-[0.16em] text-[#21161e]/55">Shop this video</p>{taggedProducts.map((product) => <Link key={product.id} href={`/shop/${product.id}`} onClick={() => trackShopClick(video.id, product.id)} className="block px-2 py-2 text-xs font-medium text-[#251521] transition hover:bg-[#f8edf3] hover:text-[#b51863]">{product.name}</Link>)}</div>}</div>}</div>; })}</div>{typeof document !== "undefined" && viewer ? createPortal(viewer, document.body) : null}</section>;
}
