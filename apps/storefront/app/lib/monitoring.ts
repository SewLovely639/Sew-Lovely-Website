...
export async function captureException(error: unknown, context: MonitoringContext = {}) {
  ...
  try {
    ...
    const event = {
      ...
      exception: { values: [{ type: exception.name, value: exception.message, stacktrace: exception.stack ? { frames: [] } : undefined }] },
    };
    const envelope = `${JSON.stringify({ event_id: eventId, sent_at: new Date().toISOString() })}\n${JSON.stringify({ type: "event" })}\n${JSON.stringify(event)}`;
    const response = await fetch(sentryEnvelopeUrl(dsn), { method: "POST", headers: { "Content-Type": "application/x-sentry-envelope" }, body: envelope });
    return response.ok;
