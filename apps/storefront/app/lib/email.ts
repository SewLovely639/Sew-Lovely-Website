...
export async function sendOrderReceipts(order: ReceiptOrder) {
  ...
  const [internal, customer] = await Promise.all([
    resend.emails.send({ from: process.env.RESEND_FROM_EMAIL, to: business, subject: `New Sew Lovely order ${order.id}`, html: html(order, "business"), headers: { "Idempotency-Key": `${idempotencyKey}-business` } }),
    resend.emails.send({ from: process.env.RESEND_FROM_EMAIL, to: order.customer.email, subject: `Your Sew Lovely order ${order.id}`, html: html(order, "customer"), headers: { "Idempotency-Key": `${idempotencyKey}-customer` } }),
  ]);
