...
export default async function Shop({ searchParams }: { searchParams: Promise<{ category?: string; brand?: string; q?: string }> }) {
  ...
  const filtered = search
    ? products.filter((product) => `${product.name} ${product.description} ${product.category} ${product.brand}`.toLowerCase().includes(search))
    : value
      ? products.filter((product) => product[field] === value)
      : products;
  ...
  return (
    <>
      ...
      <BackArrow />
      <main className="wrap catalogue-page">
        <p className="eyebrow">Shop Sew Lovely</p>
        <h1>{value ?? "All products"}</h1>
        <p>{filtered.length} {filtered.length === 1 ? "piece" : "pieces"} available</p>
        <div className="product-grid">{filtered.map((product) => <ProductCard key={product.id} product={product} />)}</div>
        {filtered.length === 0 && <p className="empty">No products are published in this search or category yet.</p>}
      </main>
      <FloatingWhatsApp />
      <footer>
